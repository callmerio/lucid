...
export default defineContentScript({
  ...
  main() {
    ...
    async function loadConfiguration() {
      try {
        ...
        if (typeof browser !== 'undefined' && browser.storage && browser.storage.local) {
          ...
          wordMarkingsData = result.wordMarkings as { [key: string]: number } | undefined || null;
        } else {
        ...
        console.log('Lucid: Configuration loaded', { highlightBaseColor, wordMarkings });
      } catch (error) {
      ...
      try {
        ...
        if (typeof browser !== 'undefined' && browser.storage && browser.storage.local) {
          ...
          if (typeof storage !== 'undefined' && storage.setItem) {
            // 这个分支理论上不应该执行
            console.warn('Lucid: Attempting to use wxt/storage for saving as a fallback (unexpected).');
            // @ts-ignore
            await storage.setItem('local:wordMarkings', wordMarkings);
          } else {
            console.error('Lucid: Storage API (browser.storage.local or wxt/storage) is not available for saving.');
            return;
          }
        }
    ...
    function expandSelectionToFullWord() {
      ...
      let specialPatternCheckStart = checkSpecialPatterns(startNode, startOffset, 'backward');
      ...
      let specialPatternCheckEnd = checkSpecialPatterns(endNode, endOffset, 'forward');
      ...
      if (wordToMark) {
        ...
        try {
          ...
          console.log('Lucid: Highlight applied for word:', wordToMark, 'Class:', className);
        } catch (e) {
          console.error("Lucid: Error surrounding contents with highlight element", e);
        }
      } else {
