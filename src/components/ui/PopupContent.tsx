...
export const PopupContent: React.FC<PopupContentProps> = ({ word, wordData, onClose }) => {
  ...
  const renderPhonetic = (region: 'us' | 'uk', phonetic: string) => (
    <div className={`lucid-toolpopup-phonetic-group ${region}-phonetic`}>
      <span className="lucid-toolpopup-phonetic-region">{region.toUpperCase()}</span>
        ...
        {wordData.explain.map((explanation, index) => (
          <div key={index} className="lucid-toolpopup-explain-group">
            <div className="lucid-toolpopup-definition">
              <span className={`lucid-toolpopup-pos pos-${explanation.pos.toLowerCase().replace(' ', '-')}`}>{explanation.pos}</span>
              {explanation.definitions.map((def, defIndex) => (
                <span key={defIndex} className="lucid-toolpopup-definition-text-chinese">
                  {def.chinese_short || def.chinese}
                  <span className="lucid-toolpopup-definition-text-english-tooltip">{def.definition}</span>
                </span>
              ))}
            </div>
          </div>
