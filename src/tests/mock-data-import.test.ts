...
import testMockData from './mock-data/tooltip-mock-data.json';

describe('Mock Data 导入方式测试', () => {
  test('方式1: 直接静态导入 JSON 文件', () => {
    expect(testMockData).toBeDefined();
    expect(testMockData.words).toHaveLength(3);
    expect(testMockData.words[0].word).toBe('escalade');
  ...
  test('方式2: 动态导入 JSON 文件', async () => {
    try {
      // 动态导入
      const mockData = await import('./mock-data/tooltip-mock-data.json');
